package com.evanoman.coursework.pcd.parallel.wk4;
		...
		for (int ii = 0; ii < tasks; ii++)
		{
			...
			threads[ii] = new Thread(() -> {
				...
				for (int iter = 0; iter < iterations; iter++)
				{
					...
					myNew[left] = (myVal[left - 1] + myVal[left + 1]) / 2d;

					final int right = (i + 1) * (n / tasks);
					myNew[right] = (myVal[right - 1] + myVal[right + 1]) / 2d;
											   ...
											   final int tasks)
	{
		...
		for (int ii = 0; ii < tasks; ii++)
		{
			...
			threads[ii] = new Thread(() -> {
				...
				{
					final int left = i * (n / tasks) + 1;
					threadPrivateMyNew[left] = (threadPrivateMyVal[left - 1] + threadPrivateMyVal[left + 1]) / 2d;

					final int right = (i + 1) * (n / tasks);
					threadPrivateMyNew[right] = (threadPrivateMyVal[right - 1] + threadPrivateMyVal[right + 1]) / 2d;
